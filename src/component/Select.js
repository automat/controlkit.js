...
function Select(parent, object, value, params) {
    ...
    var select = this._select = new Node(NodeType.INPUT_BUTTON);
        select.setStyleClass(CSS.Select);
        select.addEventListener(NodeEventType.MOUSE_DOWN, this._onSelectTrigger.bind(this));
...
var base = BaseShared.get();
        base.addEventListener(EventType.TRIGGER_SELECT, this, 'onSelectTrigger');

    this.addEventListener(EventType.SELECT_TRIGGERED, base, 'onSelectTriggered');
}
...
Select.prototype._onSelectTrigger = function () {
    this.dispatchEvent(new Event_(this, EventType.SELECT_TRIGGERED, null));
};
