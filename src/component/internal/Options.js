...
function Options(parentNode)
{
    ...
    var node     = this._node = new Node(NodeType.DIV);
    var listNode = this._listNode = new Node(NodeType.LIST);
    ...
    document.addEventListener(DocumentEventType.MOUSE_DOWN,this._onDocumentMouseDown.bind(this));
...
Options.prototype =
{
    ...
    build : function(entries,selected,element,callbackSelect,callbackOut,paddingRight,areColors,colorMode)
    {
        ...
        if(areColors)
        {
            ...
            {
                entry    = entries[i];
                itemNode = listNode.addChild(new Node(NodeType.LIST_ITEM));
                color    = itemNode.addChild(new Node(NodeType.DIV));

                switch(colorMode)
                {
                    case ColorMode.HEX:   nodeColor = entry; break;
                    case ColorMode.RGB:   nodeColor = ColorUtil.RGB2HEX(  entry[0],entry[1],entry[2]); break;
                    case ColorMode.RGBfv: nodeColor = ColorUtil.RGBfv2HEX(entry[0],entry[1],entry[2]); break;
                    case ColorMode.HSV:   nodeColor = ColorUtil.HSV2RGB(  entry[0],entry[1],entry[2]); break;
                }
                ...
                itemNode.addEventListener(NodeEventType.MOUSE_DOWN,
                    function()
                    {
...
//};
//
//ControlKit.Options.setup        = function(parentNode){return ControlKit.Options._instance = new ControlKit.Options(parentNode);};
