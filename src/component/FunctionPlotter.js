var Plotter = require('./internal/Plotter');
var ErrorUtil = require('../core/error/ckErrorUtil');
...
var FunctionPlotType = require('./internal/FunctionPlotType');
var Preset = require('../core/Preset');
...
function FunctionPlotter(parent, object, value, params) {
    ...
    var sliderXWrap = new Node(NodeType.DIV);
    sliderXWrap.setStyleClass(CSS.GraphSliderXWrap);

    var sliderYWrap = new Node(NodeType.DIV);
    sliderYWrap.setStyleClass(CSS.GraphSliderYWrap);

    var sliderXTrack = this._sliderXTrack = new Node(NodeType.DIV);
    sliderXTrack.setStyleClass(CSS.GraphSliderX);

    var sliderYTrack = this._sliderYTrack = new Node(NodeType.DIV);
    sliderYTrack.setStyleClass(CSS.GraphSliderY);

    var sliderXHandle = this._sliderXHandle = new Node(NodeType.DIV);
    sliderXHandle.setStyleClass(CSS.GraphSliderXHandle);

    var sliderYHandle = this._sliderYHandle = new Node(NodeType.DIV);
    ...
    sliderXHandle.addEventListener(NodeEventType.MOUSE_DOWN, this._onSliderXHandleDown.bind(this));
    sliderYHandle.addEventListener(NodeEventType.MOUSE_DOWN, this._onSliderYHandleDown.bind(this));
    ...
    this._unitsMinMax = [Preset.FUNCTION_PLOTTER_UNIT_MIN,
        Preset.FUNCTION_PLOTTER_UNIT_MAX]; //1/8->4

    this._scaleMinMax = [Preset.FUNCTION_PLOTTER_SCALE_MIN,
        Preset.FUNCTION_PLOTTER_SCALE_MAX]; //1/50 -> 25
    ...
    svg.addEventListener(DocumentEventType.MOUSE_DOWN, this._onDragStart.bind(this), false);
    this._wrapNode.getElement().addEventListener("mousewheel", this._onScale.bind(this, false));

    BaseShared.get().addEventListener(EventType.UPDATE_VALUE, this, 'onValueUpdate');
}
