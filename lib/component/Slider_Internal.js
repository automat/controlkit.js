...
Slider_Internal.prototype._update = function(){
  // Using element values rather than cached, since ControlKit's cached
  //    values don't update with panel position updates!
  var mx = Mouse.get().getX(),
    sx = this._slot.node._element.getBoundingClientRect().x,
    sw = this._slot.node.getWidth() - 6 // 6 is ControlKit's padding,
