...
function Panel(controlKit,params){
    ...
    var root = this._node = new Node(),
        head = this._headNode = new Node(),
        menu = new Node(),
        labelWrap = new Node(),
        label_ = new Node(Node.SPAN),
        wrap = this._wrapNode = new Node(Node.DIV),
        list = this._listNode = new Node(Node.LIST);

    root.setStyleClass(CSS.Panel);
    head.setStyleClass(CSS.Head);
    menu.setStyleClass(CSS.Menu);
    labelWrap.setStyleClass(CSS.Wrap);
    label_.setStyleClass(CSS.Label);
    wrap.setStyleClass(CSS.Wrap);
    list.setStyleClass(CSS.GroupList);
    ...
    if (this._parent.historyIsEnabled()) {
        var menuUndo = this._menuUndo = new Node(Node.INPUT_BUTTON);
            menuUndo.setStyleClass(CSS.ButtonMenuUndo);
            menuUndo.setStyleProperty('display', 'none');
            menuUndo.setProperty('value', History.get().getNumStates());
            menu.addChildAt(menuUndo, 0);

            menuUndo.addEventListener(NodeEvent.MOUSE_DOWN, this._onMenuUndoTrigger.bind(this));
            head.addEventListener(NodeEvent.MOUSE_OVER, this._onHeadMouseOver.bind(this));
            head.addEventListener(NodeEvent.MOUSE_OUT, this._onHeadMouseOut.bind(this))
    }
